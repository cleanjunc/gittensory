...
function parseNvmrcMajor(nvmrcContent) {
...
function findWorkspacePackageJsonPaths({ root, listDir }) {
  const paths = [];
...
/**
 * @param {{ root: string, readFile?: typeof defaultReadFile, listDir?: typeof defaultListDir }} args
 * @returns {{ failures: string[], nvmrcMajor: number, checkedPackages: string[] }}
 */
export function checkEnginesNvmrcSync({ root, readFile = defaultReadFile, listDir = defaultListDir }) {
...
function main() {
  ...
  console.log(
    `Engines/.nvmrc sync check ok: ${checkedPackages.length} package(s) with an engines.node pin all exclude Node ${nvmrcMajor + 1}+.`,
  );
}
